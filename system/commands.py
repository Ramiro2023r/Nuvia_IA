...
# Caché simple para evitar búsquedas de disco redundantes
_app_cache = {}

def _find_app_path(name: str) -> str | None:
    """Busca el ejecutable de una app en el Menú Inicio con caché."""
    name_low = name.lower()
    if name_low in _app_cache:
        return _app_cache[name_low]
    ...
    for base_path in search_paths:
        ...
        for root, dirs, files in os.walk(base_path):
            for file in files:
                if file.lower().endswith((".lnk", ".exe")) and name_low in file.lower():
                    path = os.path.join(root, file)
                    _app_cache[name_low] = path
                    return path
    return None

def _open(*args, shell=False):
    """Abre un proceso de forma segura. shell=True está prohibido para inputs dinámicos."""
    try:
        # Sanitización extra si llegara algo raro
        target = args[0]
        if isinstance(target, str) and (";" in target or "&" in target or "|" in target):
             print(f"[Nuvia Sec] Bloqueado intento de inyección en _open: {target}")
             return

        subprocess.Popen(
            target if shell else list(args),
            shell=shell,
