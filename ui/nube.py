...
from ui.chat_modal import ChatModal
...
def _render_cloud(tint=None, mouth_open=False, blink_frame=False, sparkles_data=None):
    ...
    # ═══════════════════════════════════════
    # DYNAMIC SPARKLES (drawn in _animate, not here)
    # ═══════════════════════════════════════
...
class CloudWindow:
    def __init__(self):
        ...
        self._cloud_cache = None  # Cache for the cloud body (static part)
    ...
    def create(self):
        ...
        # Crear el ChatModal (lazy, después de que root exista)
        self.chat_modal = ChatModal(self)
    ...
    def fade_out(self, callback, alpha=1.0):
        """Graciaful exit animation."""
        alpha = max(alpha - 0.05, 0.0)
        if self.root:
            self.root.wm_attributes("-alpha", alpha)
            if alpha > 0.0:
                self.root.after(30, lambda: self.fade_out(callback, alpha))
            else:
                callback()
    ...
    def _on_drag(self, e):
        ...
        # Sincronizar posición del chat modal
        if self.chat_modal:
            self.chat_modal.reposition()

    def _animate(self):
        self._step += 1
        now = time.time()
        
        # 1. Update Particles (Always dynamic)
        current_p_data = []
        ...
        # 2. Update Floating Logic
        dy = int(8 * math.sin(self._step * 0.055))
        if self._base_x is not None: 
            self.root.geometry(f"+{self._base_x}+{self._base_y + dy}")
        ...
        # 5. CACHED RENDERING
        # Only re-render the cloud body if state, mouth or blink changed
        ...
        needs_full_render = (
            self._cloud_cache is None or 
            self._last_state != self._state or 
            self._last_mouth != mouth or 
            self._last_blink != blink
        )
        
        if needs_full_render:
            # Render WITHOUT particles first to cache it
            self._cloud_cache = _render_cloud(tint=tint, mouth_open=mouth, blink_frame=blink, sparkles_data=None)
            self._last_state = self._state
            self._last_mouth = mouth
            self._last_blink = blink
            
        # Composite particles over the cached cloud
        final_img = self._cloud_cache.copy()
        if current_p_data:
            draw = ImageDraw.Draw(final_img)
            for p in current_p_data:
                sx, sy = p['x'], p['y']
                size = p['size'] * p['scale']
    ...
    def show_chat(self):
        ...
        if self.chat_modal:
            self.root.after(0, self.chat_modal.show)

    def hide_chat(self):
        """Cierra el modal de chat."""
        if self.chat_modal:
            self.root.after(0, self.chat_modal.hide)
